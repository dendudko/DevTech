...
def load_data(state_on_date, marine, create_new_clean_xlsx=False):
    if os.path.exists('../DB/clean_' + state_on_date) and not create_new_clean_xlsx:
        df = pd.read_excel('../DB/clean_' + state_on_date)
        return df
    else:
        df = pd.read_excel('../DB/' + state_on_date)
